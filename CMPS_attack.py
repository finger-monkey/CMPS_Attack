...
def train_CMPS(train_step1_loader, train_step3_loader, net, noise, epoch, optimizer,
    ...
    optimizer.zero_grad()
    optimizer.rescale()
    for i, ((input, _, pid, _), (metaTest, _, _, _)) in enumerate(zip(train_step1_loader, train_step3_loader)):
        ...
        fakePred = metaCentroids.mm(normMteFeat.t()).t()
        oneHotReal = torch.zeros(scores.t().shape).cuda()
        oneHotReal.scatter_(1, metaLab.view(-1, 1), float(1))
        labelLoss = F.relu(
            (fakePred * oneHotReal).sum(1).mean()
            - (fakePred * (1 - oneHotReal)).max(1)[0].mean()
        )
        finalLoss = loss_step3  + loss_step1 + label_loss
        finalLoss.backward()
